# Chrome ウェブストア掲載情報

---

## 日本語

### 簡単な説明（132文字以内）

Twitter/Xのタイムラインにワンクリックのブロック＆ミュートボタンを追加。ツイート・RT・引用RT・プロフィールに対応。

### 詳しい説明

Twitterの究極のブロックツール。タイムライン上のすべてのツイートにワンクリックのブロック＆ミュートボタンを追加するブラウザ拡張機能です。TwitterのUIに完全に溶け込むデザインで、違和感なく使えます。

■ 主な機能

・ワンクリックブロック — 不快なユーザーをボタン1つで即ブロック
・ワンクリックミュート — フォロー関係を維持したまま非表示に
・RT対応 — リツイーターと元の投稿者を個別にブロック・ミュート可能
・引用RT対応 — 引用ツイート内のユーザーにもボタンを表示
・ブロック状態の維持 — ページ遷移後もブロックしたツイートを自動で非表示

■ ボタンの表示場所

・タイムラインのツイート
・リツイート（RT者 + 元の投稿者）
・引用ツイート
・プロフィールページ
・おすすめユーザー（You might like）
・フォロー / フォロワー一覧
・ホバーカード（ユーザー名ホバー時のポップアップ）

■ 特徴

・TwitterのネイティブUIに完全に馴染むデザイン
・軽量で高速 — パフォーマンスに影響しません
・ブロック・ミュートボタンの表示/非表示を個別に設定可能
・フォロー中のユーザーをブロックする前に確認する機能
・統計機能 — ブロック・ミュート回数を記録
・日本語 / English 対応

---

## English

### Short Description (132 chars max)

Add one-click block & mute buttons to every tweet, retweet, quote tweet, and profile on Twitter/X. Native UI design.

### Detailed Description

The ultimate blocking tool for Twitter. Adds one-click block & mute buttons to every tweet on your timeline. Seamlessly integrates with Twitter's native UI for a distraction-free experience.

■ Key Features

・One-click Block — Instantly block unwanted users with a single click
・One-click Mute — Hide users from your timeline while keeping the follow
・Retweet Support — Block or mute retweeters and original authors separately
・Quote Tweet Support — Block or mute users inside quote tweets
・Persistent Block State — Blocked tweets stay hidden even after page navigation

■ Button Locations

・Timeline tweets
・Retweets (retweeter + original author)
・Quote tweets
・Profile pages
・Suggested users (You might like)
・Following / Followers lists
・Hover cards (user popups)

■ Why Ultimate Twitter Block?

・Native UI integration — buttons blend perfectly with Twitter's design
・Lightweight & fast — zero impact on page performance
・Customizable — show or hide block and mute buttons independently
・Follow protection — optional confirmation before blocking followed users
・Statistics — track your block and mute counts
・Supports English and Japanese
